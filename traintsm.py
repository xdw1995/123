...
sys.path.append('/data/xudw/SlowFastNetworks-master/SlowFastNetworks-master/second_operation/')
import TSM
import torch
    ...
    with torch.no_grad():
        for step, (inputs, labels) in enumerate(val_dataloader):
            ...
            print_string = 'Top-1 accuracy: {top1_acc:.2f}%, Top-5 accuracy: {top5_acc:.2f}%'.format(
                ...
                top5_acc=top5.avg)
            print(print_string)
            if int(top1.avg)>=niubi and step==len(val_dataloader)-1:
                niubi = int(top1.avg)
                torch.save(model.module.state_dict(), str(top1.avg)+'tsm.pth')
...
def main():
    ...
    val_dataloader = \
        DataLoader(
            dataset.VideoDataset(mode='validation', frame_sample_rate=1,clip_len=16),
            batch_size=16, shuffle=True, num_workers=8)
    # test_dataloader = \
    #     DataLoader(
    ...
    pretrained_dict = torch.load('/data/xudw/SlowFastNetworks-master/SlowFastNetworks-master/second_operation/39.0625weilaeltsm.pth', map_location='cpu')
    try:
