...
def submit():
    ...
    model_rgb_weilabel = load(model_rgb_weilabel,'80.06872852435353jiaochayanzhengRGB_quanshuju.pth')
    model_diff_weilabel = load(model_diff_weilabel,'78.4562516541447jiaochayanzhengdiff_quanshuju.pth')
    model_tsm = load(model_tsm,'tsmjiaocha1.pth')

    # model_rgb_old = model_rgb_old.cuda()
    # model_diff_old = model_diff_old.cuda()
    model_rgb_weilabel = model_rgb_weilabel.cuda()
    model_diff_weilabel = model_diff_weilabel.cuda()
    model_tsm = model_tsm.cuda()
    # model = nn.DataParallel(model)  # multi-Gpu
    # model_rgb_old.eval()
    # model_diff_old.eval()
    model_rgb_weilabel.eval()
    model_diff_weilabel.eval()
    model_tsm.eval()
    test = open('/data/xudw/test_cpu/xdw_baseline/data/annotations/mod-ucf101-test.txt','r')
    # f_RGB_old = open('RGB_old.txt','w')
    # f_diff_old = open('diff_old.txt', 'w')
    f_RGB_weilabel= open('RGB_85.txt', 'w')
    f_diff_weilabel = open('diff_85.txt', 'w')
    f_tsm = open('tsm_85.txt','w')
    t = test.readlines()
    # with open('submission_last.txt','w') as f:
    for step, inputs in enumerate(zip(test_dataloader_RGB_weilabel_balance_class,test_dataloader_diff_weilabel_balance_class,test_dataloader_tsm)):
        with torch.no_grad():
            # outputs_RGB_old = model_rgb_old(inputs[0].cuda())
            # outputs_diff_old = model_diff_old(inputs[1].cuda())
            outputs_RGB_weilabel = model_rgb_weilabel(inputs[0].cuda())
            outputs_diff_weilabel = model_diff_weilabel(inputs[1].cuda())
            outputs_tsm = model_tsm(inputs[2].cuda())

            # measure accuracy and record loss
            # outputs_RGB_old = torch.sum(outputs_RGB_old*0.1,dim=0)
            # outputs_diff_old = torch.sum(outputs_diff_old*0.1,dim=0)
            outputs_RGB_weilabel = torch.sum(outputs_RGB_weilabel*0.1,dim=0)
            outputs_diff_weilabel = torch.sum(outputs_diff_weilabel * 0.1, dim=0)
            outputs_tsm = torch.sum(outputs_tsm * 0.1, dim=0)
            # print(outputs_RGB_old.shape)
            # outputs = outputs_RGB_old*0.2+outputs_diff_old*0.2+outputs_RGB_weilabel*0.2 +outputs_diff_weilabel*0.2 +outputs_tsm*0.2
            ...
            #
            # f_RGB_old.write(' '.join(map(str,outputs_RGB_old.cpu().numpy())))
            # f_diff_old.write(' '.join(map(str,outputs_diff_old.cpu().numpy())))
            f_RGB_weilabel.write(' '.join(map(str,outputs_RGB_weilabel.cpu().numpy())))
            f_RGB_weilabel.write('\n')
            f_diff_weilabel.write(' '.join(map(str,outputs_diff_weilabel.cpu().numpy())))
            f_diff_weilabel.write('\n')
