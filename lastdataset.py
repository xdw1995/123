...
class VideoDataset(Dataset):
    ...
    def __init__(self, mode='train', clip_len=64, frame_sample_rate=1):
        ...
        if mode == 'train':
            path = '/data/xudw/test_cpu/data_new/train22.txt'
        if mode == 'validation':
            path = '/data/xudw/test_cpu/data_new/validation.txt'
        if self.mode != 'test':
    ...
    def __getitem__(self, index):
        ...
        while buffer.shape[0] < self.clip_len + 2:
            buffer = np.concatenate([buffer, buffer[::-1]])
        # while buffer.shape[0] < self.clip_len + 2:
...
class VideoDataset_RDBdiff(Dataset):
    ...
    def __init__(self, mode='train', clip_len=65, frame_sample_rate=1):
        ...
        if mode == 'train':
            path = '/data/xudw/test_cpu/data_new/train22.txt'
        if mode == 'validation':
            path = '/data/xudw/test_cpu/data_new/weitest.txt'
        if self.mode != 'test':
            with open(path, 'r') as f:
                for i in f.readlines():
                    # print(video_path+'/'+i.split(' ')[0])
...
class VideoDataset_TSM(Dataset):

    def __init__(self, mode='train', clip_len=1, frame_sample_rate=1):
        self.clip_len = clip_len
        self.short_side = [128, 160]
        self.crop_size = 112
        # self.short_side = [256, 320]
        ...
        # print(1)
        if mode == 'train':
            path = '/data/xudw/test_cpu/xdw_baseline/data/annotations/mod-ucf101-train123123.txt'
        if self.mode != 'test':
            with open(path, 'r') as f:
                for i in f.readlines():
                    capture = cv2.VideoCapture(video_path + '/' + i.split(' ')[0].strip())
                    print(video_path + '/' + i.split(' ')[0].strip())
                    retaining = True
                    while retaining:
                        retaining, frame = capture.read()
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            buffer = cv2.flip(buffer, flipCode=1)
...
if __name__ == '__main__':
    ...
    for step, (buffer) in enumerate(train_dataloader):
        print(buffer.shape)
        if step ==5:
            break
        # print("label: ", label)
