    ...
    with torch.no_grad():
        for step, (inputs, labels) in enumerate(val_dataloader):
            ...
            if int(top1.avg)>=niubi and step==len(val_dataloader)-1:
                niubi = int(top1.avg)
                torch.save(model.module.state_dict(), str(top1.avg)+'diff.pth')
...
def main():
    ...
    train_dataloader = \
        DataLoader(
            dataset.VideoDataset_RDBdiff(mode='train', frame_sample_rate=1),
            batch_size=32, shuffle=True, num_workers=8)

    val_dataloader = \
        DataLoader(
            dataset.VideoDataset_RDBdiff(mode='validation', frame_sample_rate=1),
            batch_size=32, shuffle=True, num_workers=8)
    # test_dataloader = \
    #     DataLoader(
    ...
    pretrained_dict = torch.load('/data/xudw/SlowFastNetworks-master/SlowFastNetworks-master/slowfast_RGBdiff_pool_ELU_xinshuju.pth.tar', map_location='cpu')
    try:
