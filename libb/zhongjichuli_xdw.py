...
class RandomHorizontalFlip(Transform):
    ...
    def __call__(self, data):
        if self.rng.rand() < 0.5:
            # print(data.shape,"111")
            data = np.fliplr(data)
            data = np.ascontiguousarray(data)

            # print(data.shape,"222")
        return data
...
video_transform=Compose([
    RandomScale(make_square=False,
                           aspect_ratio=[0.8, 1./0.8],
                           slen=[144, 160]),
    RandomCrop((112, 112)), # insert a resize if needed  #todo TSM  no slowfast
...
test_transform=Compose([
    Resize((144, 160)),
    CenterCrop((112, 112)),  #transforms.RandomCrop((224,224)),test#todo TSM  no slowfast
    ToTensor(),
    normalize,
])
"""
