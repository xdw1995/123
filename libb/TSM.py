...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        if self.dropout == 0:
            ...
            self.new_fc_xdw = nn.Linear(feature_dim, num_class)

            std = 0.001
            normal_(self.new_fc_xdw.weight, 0, std)
            constant_(self.new_fc_xdw.bias, 0)
    ...
    def forward(self, input, no_reshape=False):
        ...
        if self.dropout > 0:
            base_out = self.new_fc_xdw(base_out)
        ...
        if self.reshape:
            if self.is_shift and self.temporal_pool:
                ...
                # print((-1, self.num_segments) + base_out.size()[1:])
...
# net =TSM(10,11)
# # print(net)
# model = TSN(101,16,modality = 'RGB',partial_bn=True)
# #--gd 20 --lr 0.02 --wd 1e-4 --lr_steps 20 40 --epochs 50 \
# a = torch.ones(1,3,16,224,224)
#
# print(model(a).shape)
# import torchvision
...
def diff(need_3D=True):
    ...
    print(x.shape)
    return x
# model = TSN(101,16,modality='RGB',is_shift=True,pretrain=None)
# model.load_state_dict(torch.load('123.pkl'))
# print(model(torch.ones((1,3,16,112,112))).shape)
