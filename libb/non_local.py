...
class NL3DWrapper_xdw(nn.Module):
    def __init__(self, block, out_filters):
        super(NL3DWrapper_xdw, self).__init__()
        self.block = block
        self.nl = NONLocalBlock3D(out_filters)
...
if __name__ == '__main__':
    ...
    # f = 8
    # conv = nn.Conv3d(3, f, kernel_size=(3, 1, 1), stride=(2, 1 ,1), bias=False, padding=(1, 0, 0))
    # #conv 或者 block
    # img = torch.randn(2, 3, 10, 20, 20)
    # print(conv(img).shape)
    # net = NL3DWrapper_xdw(conv,f)
    # # net = NONLocalBlock3D(3, sub_sample=sub_sample, bn_layer=bn_layer)
    # out = net(img)
    # print(out.size())
    # # make_non_local(torchvision.models.resnet50(False), n_segment =16)
    #densenet 融合 +non_local
    # from libb import slowfastnet
    # model = slowfastnet.resnet101(non_local = True)
    # print(model(torch.ones(1,3,64,224,224)).shape)
